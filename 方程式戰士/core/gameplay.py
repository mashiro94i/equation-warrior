...
def placement_zone_around_player(
    player_cx: float,
    screen_width: int,
    screen_height: int,
) -> Tuple[int, int, int, int]:
    """可放置區：螢幕寬 1/2、全高，水平以玩家為中心（貼邊時夾在螢幕內）。"""
    w = max(1, screen_width // 2)
    x = int(round(player_cx)) - w // 2
    if x < 0:
        x = 0
    if x + w > screen_width:
        x = max(0, screen_width - w)
    return (x, 0, w, screen_height)


def placement_zone_left_third(screen_width: int, screen_height: int) -> Tuple[int, int, int, int]:
    """相容舊名：以螢幕水平 1/6 為中心（測試／舊呼叫）。"""
    return placement_zone_around_player(screen_width / 6.0, screen_width, screen_height)


def point_in_placement_zone(
    px: float,
    py: float,
    player_cx: float,
    screen_width: int,
    ...
    x, y, w, h = placement_zone_around_player(player_cx, screen_width, screen_height)
