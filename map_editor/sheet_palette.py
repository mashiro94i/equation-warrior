"""左欄圖集：依各 Kenney 包規則從 tile PNG 合成（不使用 Preview.png）。"""
...
def _pixel_platformer_tile_rows(pack_dir: Path) -> list[list[Path]]:
    """先整列 Backgrounds、再整列 Characters、其餘 Tiles 每列 20 個。"""
...
def fit_zoom(state: PageState, vp_w: int, vp_h: int) -> float:
    sw, sh = state.surf.get_size()
    if sw < 1 or sh < 1:
        return 1.0
    return min(vp_w / sw, vp_h / sh) * 0.96
